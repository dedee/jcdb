    ...
    public static void setupTestData() throws IOException {
        ...
        try (CdbWriter writer = new CdbWriter(TEST_CDB_PATH)) {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                for (int j = 0; j < 5; j++) {
                    writer.add(key.getBytes(), ("multi-value-" + i + "-" + j).getBytes());
                }
            }
            ...
            for (int i = 0; i < 50; i++) {
                ...
                writer.add(longKey.getBytes(), longValue.getBytes());
            }

            // Pattern 4: Keep some of the original test keys for backwards compatibility
            writer.add("test-key-0".getBytes(), "value-0".getBytes());
            writer.add("test-key-1".getBytes(), "value-1a".getBytes());
            writer.add("test-key-1".getBytes(), "value-1b".getBytes());
            writer.add("test-key-2".getBytes(), "value-2".getBytes());
        }
    }
